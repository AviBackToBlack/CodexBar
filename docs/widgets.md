...
- Claude Usage widgets show each known model-scoped weekly quota after the normal Session, Weekly, and Opus rows.
  This includes Fable when Claude exposes it. To hide these rows, use **Preferences → Providers → Claude → Show
  model-specific weekly usage in widgets**; the setting is on by default and does not affect fetching or other
  CodexBar surfaces.
