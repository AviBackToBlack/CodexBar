...
extension CodexBarCLI {
        ...
        error: Error?) -> CostPayload
    {
        ...
        return CostPayload(
            ...
            provenance: snapshot.map { $0.summary(forLastDays: $0.historyDays).provenance.rawValue },
            coverage: snapshot.map { $0.summary(forLastDays: $0.historyDays).coverage },
            error: error.map { Self.makeErrorPayload($0) })
    }

    static func makeOpenCodexCostPayload(snapshot: CostUsageTokenSnapshot) -> CostPayload {
        ...
        now: Date = Date()) -> CostPayload?
    {
        ...
        let store = OpenCodexUsageStore(cacheRoot: OpenCodexUsageLog.cacheRoot(environment: environment))
        ...
        return self.makeOpenCodexCostPayload(snapshot: snapshot)
    }
