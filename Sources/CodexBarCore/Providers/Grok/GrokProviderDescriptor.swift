...
struct GrokWebFetchStrategy: ProviderFetchStrategy {
 ...
 func fetch(
  ...
  let authCredentials = GrokSettingsReader.resolvedCredentials(environment: context.env).flatMap {
   credentials in
 ...
 func fetchWebBilling(
  ...
  proxyBilling: ProxyBillingFetch = { try await GrokCreditsProxyFetcher.fetch(credentials: $0) }
 ) async throws
  -> (
   snapshot: GrokWebBillingSnapshot,
   sourceLabel: String,
 ...
 private func fetchLegacyWebBilling(
  context: ProviderFetchContext,
  browserCredentials: GrokCredentials?
 ) async throws -> (
  snapshot: GrokWebBillingSnapshot,
  sourceLabel: String,
  authenticatedByAuthFile: Bool
 ...
 static func credentialsForWebBillingSnapshot(
  ...
  static func fetchFirstValidCookieSession(
   ...
   fetch: ((String, GrokCredentials?) async throws -> GrokWebBillingSnapshot)? = nil
  ) async throws
